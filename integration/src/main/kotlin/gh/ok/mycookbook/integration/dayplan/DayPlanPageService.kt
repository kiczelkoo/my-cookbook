    ...
    fun getAllMealsForADay(driver: ChromeDriver, date: LocalDate): List<String> {
        ...
        try {
            ...
            if (!isProperPageOpened(driver, date)) {
                println(
                    "not correct url ${driver.currentUrl} ${DAY_PLAN_BASE_URL.plus(
                        DateCalculator.toString(date)
                    )}"
                )
                return emptyList()
            }
        ...
        try {
            ...
            wait.until(
                ExpectedConditions.numberOfElementsToBe(
                    By.cssSelector(KCAL_VALUES_SELECTOR),
                    4
                )
            )
            if (!isProperPageOpened(driver, date)) {
            ...
            summary.addAll(
                driver.findElementsByCssSelector(DAY_PLAN_SUMMARY_SELECTOR).map { it.text })
            return summary
        } catch (exc: Exception) {
